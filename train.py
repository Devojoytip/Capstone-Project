...
def get_parser():
    parser = argparse.ArgumentParser(
        ...
        description='This module demonstrates shadow detection and removal using GAN.',
        add_help=True)
...
def main(parser):
    ...
    if parser.load is not None:
        ...
        D1_weights = torch.load('./checkpoints/GAN_D1_'+parser.load+'.pth')
        D1.load_state_dict(fix_model_state_dict(D1_weights))

        D2_weights = torch.load('./checkpoints/GAN_D2_'+parser.load+'.pth')
    ...
    G1_update, G2_update, D1_update, D2_update = train_model(G1, G2, D1, D2, dataloader=train_dataloader,
                                                            ...
                                                            parser=parser, save_model_name='GAN')
