...
def get_parser():
    parser = argparse.ArgumentParser(
        prog='2 Tier GAN: Conditional Generative Adversarial Networks for Jointly Learning Shadow Detection and Shadow Removal',
        usage='python3 main.py',
        description='This module demonstrates shadow detection and removal using 2 Tier GAN.',
        add_help=True)
...
def calculate_inception_score(p_yx_list, eps=1E-16):
    all_kl_d = []
    for p_yx in p_yx_list:
        # calculate p(y)
        p_y = np.expand_dims(p_yx.mean(axis=0), 0)
        sum_kl_d = p_y.sum(axis=1)
        avg_kl_d = np.mean(sum_kl_d)
        all_kl_d.append(avg_kl_d)
    # calculate the mean of all the average KL divergences
    mean_kl_d = np.mean(all_kl_d)
    # undo the logs
    is_score = np.exp(mean_kl_d)
    return is_score
...
def test(G1, G2, test_dataset):
    ...
    for n, (img, gt_shadow, gt) in enumerate([test_dataset[i] for i in range(test_dataset.__len__())]):
        ...
        grid = make_grid(torch.cat([unnormalize(img), unnormalize(gt),
        ...
        gt_shadow = make_grid([unnormalize(torch.cat([gt_shadow]))])
        save_image(gt_shadow, './test_result/detected_shadow/'+test_dataset.img_list['path_A'][n].split('/')[4])

        output_image = gt
        p_yx = torch.abs(output_image)
        p_yx = np.array(p_yx)
        p_yx_list.append(p_yx)
        
        gt = transforms.ToPILImage(mode='RGB')(unnormalize(gt)[0, :, :, :])
        gt.save('./test_result/shadow_removal_image/'+test_dataset.img_list['path_A'][n].split('/')[4])
      
    score = calculate_inception_score(p_yx_list)
    print('Inception Score = ',score) # 1.7405291
...
def main(parser):
    ...
    if parser.load is not None:
        ...
        G1_weights = torch.load('./checkpoints/GAN_G1_'+parser.load+'.pth')
        G1.load_state_dict(fix_model_state_dict(G1_weights))

        G2_weights = torch.load('./checkpoints/GAN_G2_'+parser.load+'.pth')
